...
🤖 [Moltbot](https://molt.bot)（自托管AI助手，可接入多平台）

🎬 [Remotion](https://www.remotion.dev/)（用React代码生成视频）

🤝 [Cowork](https://github.com/shiroyk/Cowork)（实时协作编辑系统）

🧩 [OpenCode](https://opencode.ai/)（开源AI编码代理）
...
- [教程示例](#教程示例)
    ...
    - [构建自己的 `云游戏/云手机平台`](#构建自己的-云游戏云手机平台)
    - [构建自己的 `Git`](#构建自己的-git)
    - [构建自己的 `算法`](#构建自己的-算法)
    - [构建自己的 `大语言模型与训练workflow`](#构建自己的-大语言模型与训练workflow)
    - [构建自己的 `AI 搜索引擎` (Perplexity 替代品)](#构建自己的-ai-搜索引擎-perplexity-替代品)
    - [构建自己的 `AI 智能体 (Agent)` (Devin/Claude Cowork 替代品)](#构建自己的-ai-智能体-agent-devinclaude-cowork-替代品)
    - [构建自己的 `AI 知识库与播客` (NotebookLM 替代品)](#构建自己的-ai-知识库与播客-notebooklm-替代品)
    - [构建自己的 `AI 角色扮演伴侣` (Character.ai 替代品)](#构建自己的-ai-角色扮演伴侣-characterai-替代品)
    - [构建自己的 `机器人与具身智能` (Unitree 替代品)](#构建自己的-机器人与具身智能-unitree-替代品)
    - [构建自己的 `大模型分布式训练` (Megatron 替代品)](#构建自己的-大模型分布式训练-megatron-替代品)
    - [构建自己的 `AI 代码编辑器` (Cursor 替代品)](#构建自己的-ai-代码编辑器-cursor-替代品)
    - [构建自己的 `AI 制药` (AlphaFold 替代品)](#构建自己的-ai-制药-alphafold-替代品)
    - [构建自己的 `AI 核聚变` (科学计算)](#构建自己的-ai-核聚变-科学计算)
    - [构建自己的 `AI 游戏制作` (不只是 NPC)](#构建自己的-ai-游戏制作-不只是-npc)
    - [构建自己的 `AI 漫剧制作` (Comic Factory)](#构建自己的-ai-漫剧制作-comic-factory)
    - [构建自己的 `AI 量化交易` (Quant/FinRL)](#构建自己的-ai-量化交易-quantfinrl)
    - [构建自己的 `LLMOps` (LangSmith/Langfuse 替代品)](#构建自己的-llmops-langsmithlangfuse-替代品)
    - [构建自己的 `Kubernetes 管理后台` (Dashboard 替代品)](#构建自己的-kubernetes-管理后台-dashboard-替代品)
    - [构建自己的 `即时通讯与团队协作` (微信/Discord 替代品)](#构建自己的-即时通讯与团队协作-微信discord-替代品)
    - [构建自己的 `商业运营与数据分析` (SaaS 替代品)](#构建自己的-商业运营与数据分析-saas-替代品)
...
* [**多语言**：_CloudGamePlatform：云游戏/云电脑全栈方案_](https://github.com/hubin858130/CloudGamePlatform)
* [**多语言**：_cloud-game：Web 端复古游戏云平台_](https://github.com/giongto35/cloud-game)
* [**多语言**：_CloudPhone：云手机 PaaS_](https://github.com/lloves/CloudPhone)
